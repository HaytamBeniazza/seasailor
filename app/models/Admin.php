<?php
  class Admin {
    ...
    {
      $this->db->query('INSERT INTO ship (shipname, rooms, places) VALUES(:name, :rooms, :places)');
      // Bind values
    ...
    public function getcruise() {
      $results = $this->db->query('SELECT * FROM `cruise` JOIN `port` on port.idport = cruise.idport JOIN ship on ship.idship = cruise.idship ');
    ...
    {
      $this->db->query('DELETE FROM `ship` WHERE `ship`.`idship` = :idship');
    ...
    {
      $this->db->query('DELETE FROM `port` WHERE `port`.`idport` = :idport');
      $this->db->bind(':idport', $idport);
      if ($this->db->execute()) {
        return true;
      } else {
        return false;
